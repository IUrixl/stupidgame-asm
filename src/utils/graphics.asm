...
clear_back:
	...
	mov ax, [graphics.back_buffer]
	mov es, ax
	mov di, 0 ; partimos desde el pixel 0
...
render_sprite: ;ax = offset_x ; bx = offset_y, ; dl = asset_id
	...
	mov word [graphics.rendering_offy], bx ;
	...
	sprite:
		...
		mov cx, 0 ; contador x respecto el width
		mov dx, 0 ; contador y
		.plot:
			...
			.process_plot:
				...
				mov bx, dx
				add bx, [graphics.rendering_offy] ; le sumamos el offset y del sprite
				...
				cmp bx, 200 ; lo mismo que la x pero con y
				jge .prepare_next
				cmp bx, 0
				jl .prepare_next
...
vsync:
	...
	mov dx, 0x3DA ; tarjeta vga
	in al, dx
	test al, 8 ; bit de redibujo vertical
	pop ax
	pop dx
...
render_buffer: ; dibujamos el backbuffer en el VGA
	push ax
	push bx
	push cx
	push es
	...
	mov ax, [graphics.back_buffer]
	mov bx, [graphics.vga_segment]
	mov cx, 32000 ; 64k bytes en words son 32k words

	xor di, di
	xor si, si ; a 0 los 0 offsets

	mov es, bx ; vga
	mov ds, ax ; back buffer

	cld ; ponemos la df a 0
	rep movsw ; [es:di] toma el valor de [ds:si]
